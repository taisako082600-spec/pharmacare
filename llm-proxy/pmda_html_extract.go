...
// 11.2「その他の副作用」(アンカーは HDR_OtherAdverseEvents)は意図的に取り込んでいない。
//
// 介護職が日々見るのはこちら側(発疹・下痢・眠気など)なので内容としては欲しいが、
// 実データで測ると**表が壊れた状態でしか取れない**(2026-08-27、ロキソニン錠60mgで確認)。
// 元は「頻度 × 器官分類」の表で、テキスト化すると
//
//	0.1～2%未満 / 0.1%未満 / 頻度不明 / 過敏症 / 発疹、そう痒感 / …
//
// のように見出しと症状が切り離され、どの症状がどの頻度なのか読めなくなる。
// この状態でLLMに整理させると、頻度と症状の対応を推測で埋めることになり、
// 「LLMに医学的判断をさせない」という本アプリの前提を破る。しかも出力は
// 整って見えるため誤りに気づけない。
//
// 分量は問題ではない(11.2は262文字。11.1の1,190文字よりむしろ短い)。
// 取り込むなら先に <table> の行構造を保ったまま抽出する実装が要る。
