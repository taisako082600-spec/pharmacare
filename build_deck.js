...
// 厚労省側の表(二要素認証・権限・ログ)を、専門用語を使わずに開く。
explainerSlide(
  '入れる人を絞り、触った跡を残す',
  '前ページの「二要素認証・アクセス権限・監査ログ」は、要するにこの3つです',
  [
    [
      '二要素認証', 'パスワードだけでは入れない',
      'パスワードは漏れます。\n使い回していれば、別のサービス\nから漏れた1つで入られます。\n知識だけで通れるのが弱点です。',
  ...
  'あわせて、15分操作がないと自動でサインアウトします。共用端末に前の人の画面が残らないようにするためです。',
);
...
// ══════════════════════════════════ 電子保存の3原則をかみ砕く
//
// 前の2枚は「条文 → 実装」の対応表だが、実装側の語が専門的で
// 「で、何をしたの?」が残る。ここから2枚かけて、表の各行を
// 何のための決まりか → このアプリで何をしたか、の順に開く。
// システムに詳しくない人がこの資料だけで理解できることを基準にする。
function explainerSlide(title, sub, items, footer) {
  const s = light();
  heading(s, title, sub);

  items.forEach(([name, lead, why, how], i) => {
    const x = 0.8 + i * 4.05;
...
//
// 置き場所: 直前の表の1行目(6.1 国内法の執行が及ぶ範囲に置く)の説明なので、
// その表の直後に置く。表 → 6.1の説明 → 6.2の説明、と条文の並び順どおりに読める。
// 以前は3原則の解説を挟んでいて、6.1だけ2枚先まで説明が出てこなかった。
{
  const s = light();
  heading(s, 'どこに置いて、なぜそこなのか',
    '前ページ 6.1「国内法の執行が及ぶ範囲に置く」が、実際にどうなっているか');
  ...
  s.addText('※ 基盤事業者は Google Cloud Japan ですが、米国法の適用可能性は完全には排除できません。当社自身のPマーク／ISMSも未取得です（この章の最後に記載）。', {
...
// 6.1(前々ページ)に続いて、同じ表の残りの行をここで開く。
explainerSlide(
  '紙のカルテと同じ信頼を、データで担保する',
  '同じ表の「真正性・見読性・保存性」は、かみ砕くとこの3つです',
  [
    [
